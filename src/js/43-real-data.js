...
// ══════════════════════════════════════════════════════════════════════════════
// API KEY STORE — read from localStorage on load, persist user-entered keys
// ══════════════════════════════════════════════════════════════════════════════
const _RD_KEY_DEFS = [
  { id:'CENSUS_API_KEY', label:'Census API',  signup:'https://api.census.gov/data/key_signup.html',  critical:true,
    desc:'ACS demographics, ZBP business density, CBP, PEP, BPS (foundation of pipeline)' },
  { id:'HUD_TOKEN',      label:'HUD',         signup:'https://www.huduser.gov/portal/dataset/fmr-api.html',
    desc:'Fair Market Rents, Income Limits, USPS Vacancy' },
  { id:'BEA_API_KEY',    label:'BEA',         signup:'https://apps.bea.gov/API/signup/',
    desc:'Regional Per Capita Personal Income' },
  { id:'NOAA_TOKEN',     label:'NOAA',        signup:'https://www.ncdc.noaa.gov/cdo-web/token',
    desc:'NCEI Climate Normals 1991-2020' },
  { id:'AIRNOW_API_KEY', label:'AirNow',      signup:'https://docs.airnowapi.org/account/request/',
    desc:'EPA Air Quality Index' },
  { id:'BLS_API_KEY',    label:'BLS',         signup:'https://data.bls.gov/registrationEngine/',
    desc:'BLS OES occupation wages (no key works for low volume)' },
  { id:'FRED_API_KEY',   label:'FRED',        signup:'https://fred.stlouisfed.org/docs/api/api_key.html',
    desc:'FRED county-level LAUS unemployment' },
  { id:'SAM_API_KEY',    label:'SAM.gov',     signup:'https://sam.gov/data-services/Public%20API%20%E2%80%93%20Search%20Federal%20Opportunities',
    desc:'Federal contracting opportunities' },
];
...
function rdShowApiKeysPanel() {
  if (typeof document === 'undefined') return;
  if (document.getElementById('rdApiKeysModal')) {
    document.getElementById('rdApiKeysModal').style.display = 'flex';
    return;
  }
  _rdLoadKeysFromStorage();
  const rows = _RD_KEY_DEFS.map(def => {
    const current = (typeof window !== 'undefined' && window[def.id]) ? window[def.id] : '';
    const masked = current ? current.slice(0, 4) + '…' + current.slice(-4) : '';
    return `<div style="display:grid;grid-template-columns:120px 1fr auto;gap:8px;align-items:center;padding:6px 0;border-bottom:1px solid var(--border,#2a2d35)">
      <div>
        <div style="font-size:11px;font-weight:700;color:var(--text,#eee)">${def.label}${def.critical?' <span style="color:#ef4444">🚨</span>':''}</div>
        <div style="font-size:9px;color:var(--muted,#8a8d96)">${def.desc}</div>
      </div>
      <input type="password" id="apikey-${def.id}" placeholder="${current ? 'saved: '+masked : 'paste key'}" value="${current ? '' : ''}"
        style="background:var(--surface3,#1a1d22);border:1px solid var(--border,#2a2d35);color:var(--text,#eee);padding:5px 7px;border-radius:4px;font-size:11px;font-family:monospace;width:100%" />
      <a href="${def.signup}" target="_blank" rel="noopener" style="font-size:10px;color:var(--blue,#4a9eff);text-decoration:none;white-space:nowrap">get key →</a>
    </div>`;
  }).join('');
  const modal = document.createElement('div');
  modal.id = 'rdApiKeysModal';
  modal.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px';
  modal.innerHTML = `
    <div style="background:var(--surface,#13161a);border:1px solid var(--border,#2a2d35);border-radius:12px;padding:18px 22px;max-width:640px;width:100%;max-height:85vh;overflow:auto">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
        <h3 style="margin:0;font-size:15px;font-family:'Syne',sans-serif;color:var(--text,#eee)">🔑 Free Government Data API Keys</h3>
      ...
      <div style="display:flex;gap:8px;margin-top:14px">
        <button id="rdSaveKeys" style="flex:1;background:var(--green,#22c55e);color:#000;border:none;padding:8px;border-radius:6px;font-size:11px;font-weight:700;cursor:pointer">Save Keys</button>
        <button onclick="document.getElementById('rdApiKeysModal').remove()" style="background:var(--surface3,#1a1d22);color:var(--text,#eee);border:1px solid var(--border,#2a2d35);padding:8px 14px;border-radius:6px;font-size:11px;cursor:pointer">Cancel</button>
      </div>
      <div id="rdSaveStatus" style="font-size:10px;color:var(--muted,#8a8d96);margin-top:8px;min-height:14px"></div>
    </div>`;
  document.body.appendChild(modal);
  document.getElementById('rdSaveKeys').onclick = () => {
    let saved = 0;
    for (const def of _RD_KEY_DEFS) {
...
// Auto-load on script parse so prefetchRealData sees the keys
_rdLoadKeysFromStorage();

if (typeof window !== 'undefined') {
  window.rdShowApiKeysPanel = rdShowApiKeysPanel;
}
...
//   • otherwise                 → direct fetch with client-side key injection
//
// Sources accepted: census, hud, bea, noaa, airnow, bls, fred, sam, nrel, fbi
// `path` is the URL path relative to the source's base (must start with '/').
...
async function _rdGovFetch(source, path, opts = {}) {
  ...
  const SOURCES = {
    census: { base:'https://api.census.gov',        key:'CENSUS_API_KEY',  mode:'query',  name:'key' },
    hud:    { base:'https://www.huduser.gov',       key:'HUD_TOKEN',       mode:'header', name:'Authorization', prefix:'Bearer ' },
    bea:    { base:'https://apps.bea.gov',          key:'BEA_API_KEY',     mode:'query',  name:'UserID' },
    noaa:   { base:'https://www.ncei.noaa.gov',     key:'NOAA_TOKEN',      mode:'header', name:'token' },
    airnow: { base:'https://www.airnowapi.org',     key:'AIRNOW_API_KEY',  mode:'query',  name:'API_KEY' },
  ...
  if (cfg.mode === 'query') {
    url += (url.includes('?') ? '&' : '?') + cfg.name + '=' + encodeURIComponent(k);
  } else if (cfg.mode === 'header') {
    headers[cfg.name] = (cfg.prefix || '') + k;
  } else if (cfg.mode === 'body' && method === 'POST') {
    headers['Content-Type'] = 'application/json';
...
// ── _rdFetchWithProxy ─────────────────────────────────────────────────────────
// Intercepting wrapper used by existing fetchers — when USE_PROXY is true and
// the URL matches a known source base, the request is rewritten through
// /api/proxy with the key stripped client-side. Otherwise the original
// fetch(url, opts) is executed as-is. Lets existing fetchers stay untouched.
const _RD_PROXY_MAP = {
  'https://api.census.gov':          'census',
  'https://www.huduser.gov':         'hud',
  'https://apps.bea.gov':            'bea',
  'https://www.ncei.noaa.gov':       'noaa',
  'https://www.airnowapi.org':       'airnow',
  'https://api.bls.gov':             'bls',
  'https://api.stlouisfed.org':      'fred',
  'https://api.sam.gov':             'sam',
  'https://developer.nrel.gov':      'nrel',
...
async function _rdFetchWithProxy(url, opts = {}) {
  ...
  let path = url.slice(baseMatch.length);
  // Strip key from URL — server proxy adds it
  path = path.replace(/[?&](key|api_key|UserID|API_KEY)=[^&]*/gi, m => m[0] === '?' ? '?' : '&');
  path = path.replace(/\?&/, '?').replace(/&$/, '').replace(/\?$/, '');
  if (!path.startsWith('/')) path = '/' + path;
  return _rdViaProxy(source, path, {
...
async function prefetchRealData(zipCode, industryKey, capacityVal, budgetVal) {
  ...
  const [
    ...
    (lat && lng) ? _rdFetchNREL(zipCode, lat, lng)                   : Promise.resolve(null),
...
async function _rdFetchHUDRents(zip, stateAbbr) {
  ...
  try {
    ...
    const res = await _rdFetchWithProxy(url, {
...
async function _rdFetchSAMGov(naics, stateAbbr) {
  ...
  if (_rdCacheGet(k)) return _rdCacheGet(k);
  try {
    // SAM.gov requires a free API key (set window.SAM_API_KEY).
    // Without a key it returns 404 — silent-fail.
    const samKey = (typeof window !== 'undefined' && window.SAM_API_KEY) ? window.SAM_API_KEY : null;
    if (!samKey) return null;
    const today = new Date();
    const fromDate = `${(today.getMonth()+1).toString().padStart(2,'0')}/01/${today.getFullYear()-1}`;
    const url = `https://api.sam.gov/opportunities/v2/search?limit=5&postedFrom=${fromDate}&naics=${naics}&ptype=o&active=Yes&api_key=${samKey}`;
    const res = await _rdFetchWithProxy(url, { signal:_rdAbortTimeout(10000) });
    if (!res.ok) return null;
    const d = await res.json();
...
async function _rdFetchFEMAFlood(lat, lng) {
  ...
  try {
    ...
    // FEMA migrated NFHL to a hosted FeatureServer on services.arcgis.com in late 2024;
    // hazards.fema.gov/.../MapServer/28 now returns a 404 WebSEAL error.
    const url = `https://services.arcgis.com/2gdL2gxYNFY2TOUb/arcgis/rest/services/FEMA_National_Flood_Hazard_Layer/FeatureServer/0/query?geometry=${lng},${lat}&geometryType=esriGeometryPoint&inSR=4326&spatialRel=esriSpatialRelIntersects&outFields=FLD_ZONE,ZONE_SUBTY,SFHA_TF&returnGeometry=false&f=json`;
    const res = await _rdFetchWithProxy(url, { signal:_rdAbortTimeout(10000) });
    if (!res.ok) return null;
    const d = await res.json();
...
async function _rdFetchFBICrime(stateAbbr) {
  ...
  try {
    ...
    if (!res.ok) return null;
    const d = await res.json();
    // Response shape: {offenses:{rates:{"<State> Offenses":{MM-YYYY: rate, ...}}}}
    ...
    const monthRates = ratesObj[stateKey];
    const months = Object.values(monthRates).filter(v => typeof v === 'number');
    if (!months.length) return null;
    const annualRatePer100k = months.reduce((s, n) => s + n, 0); // monthly /100k rates sum ≈ annual /100k
    const result = {
      violent_per_100k:  Math.round(annualRatePer100k),
      property_per_100k: null, // separate endpoint required for property crime
      year:              2022,
      state:             stateAbbr,
      source:            'FBI CDE (api.usa.gov)',
    };
    return _rdCacheSet(k, result);
  } catch(e) { console.warn('[RealData] FBI crime failed:', e.message); return null; }
}

async function _rdFetchNREL(zip, lat, lng) {
  if (!lat || !lng) return null;
  const k = `rdnrel:${lat.toFixed(3)}:${lng.toFixed(3)}`;
  if (_rdCacheGet(k)) return _rdCacheGet(k);
  try {
...
async function _rdFetchEPAEcho(zip) {
  ...
  if (_rdCacheGet(k)) return _rdCacheGet(k);
  try {
    // EPA ECHO requires a 2-step call: get_facilities returns counts + a QueryID,
    // then get_qid pulls actual facility rows. Step 1 also returns aggregate stats
    // we can keep (total regulated, formal actions, penalties).
    const step1Url = `https://echodata.epa.gov/echo/echo_rest_services.get_facilities?output=JSON&p_zip=${zip}&p_act=Y&qcolumns=1,4,12,19`;
    const res1 = await fetch(step1Url, { signal:_rdAbortTimeout(10000) });
    if (!res1.ok) return null;
    const d1 = await res1.json();
    const r1 = d1?.Results || {};
    const qid = r1.QueryID;
    const totalRegulated = parseInt(r1.QueryRows) || 0;
    const formalActions  = (parseInt(r1.CAARows)||0) + (parseInt(r1.CWARows)||0) + (parseInt(r1.RCRRows)||0);
    let facilityNames = [];
    if (qid && totalRegulated > 0) {
      try {
        const step2Url = `https://echodata.epa.gov/echo/echo_rest_services.get_qid?qid=${qid}&qrows=1:5&output=JSON`;
        const res2 = await fetch(step2Url, { signal:_rdAbortTimeout(8000) });
        if (res2.ok) {
          const d2 = await res2.json();
          const facs = d2?.Results?.Facilities || [];
          facilityNames = facs.map(f => f.FacName || f.RegistryID).filter(Boolean);
        }
      } catch {}
    }
    const totalPenalties = (r1.TotalPenalties || '').replace(/[$,]/g, '');
    const result = {
      total_regulated: totalRegulated,
      formal_actions:  formalActions,
      total_penalties_usd: parseFloat(totalPenalties) || 0,
      caa_violations: parseInt(r1.CAARows) || 0,
      cwa_violations: parseInt(r1.CWARows) || 0,
      rcra_violations:parseInt(r1.RCRRows) || 0,
      with_violations: formalActions,
...
async function _rdFetchBLSOES(stateAbbr, industryKey) {
  ...
  try {
    ...
    const res = await _rdFetchWithProxy('https://api.bls.gov/publicAPI/v2/timeseries/data/', {
      method:'POST',
      headers:{ 'Content-Type':'application/json' },
...
async function _rdFetchNCESSchools(lat, lng) {
  ...
  if (_rdCacheGet(k)) return _rdCacheGet(k);
  try {
    // NCES EDGE Public_School_Locations_Current layer 0; 5-mile buffer (8047 m).
    // The schema does NOT include SCH_NAME/LEVEL/ENROLLMENT — only NAME/STATE/CITY/LOCALE.
    // Locale code: 11-13 city, 21-23 suburb, 31-33 town, 41-43 rural.
    const url = `https://services1.arcgis.com/Ua5sjt3LWTPigjyD/ArcGIS/rest/services/Public_School_Locations_Current/FeatureServer/0/query?geometry=${lng},${lat}&geometryType=esriGeometryPoint&distance=8047&units=esriSRUnit_Meter&inSR=4326&spatialRel=esriSpatialRelIntersects&outFields=NAME,STATE,CITY,LOCALE&returnGeometry=false&f=json&resultRecordCount=50`;
    const res = await _rdFetchWithProxy(url, { signal:_rdAbortTimeout(10000) });
    if (!res.ok) return null;
    const d = await res.json();
    ...
    const result = {
      ...
      suburb_count:        suburbCount,
      rural_town_count:    ruralCount,
      example_names:       schools.slice(0, 5).map(s => s.NAME).filter(Boolean),
...
async function _rdFetchCDCSVI(lat, lng) {
  ...
  try {
    ...
    // CDC/ATSDR SVI 2022 hosted by onemap.cdc.gov (services3.arcgis.com endpoint
    // returns 400 Invalid URL). Layer 8 = US tract level SVI 2022.
    const url = `https://onemap.cdc.gov/onemapservices/rest/services/SVI/CDC_ATSDR_Social_Vulnerability_Index_2022_USA/FeatureServer/8/query?geometry=${lng},${lat}&geometryType=esriGeometryPoint&inSR=4326&spatialRel=esriSpatialRelIntersects&outFields=FIPS,RPL_THEMES,RPL_THEME1,RPL_THEME2,RPL_THEME3,RPL_THEME4&returnGeometry=false&f=json`;
    const res = await _rdFetchWithProxy(url, { signal:_rdAbortTimeout(10000) });
    if (!res.ok) return null;
    const d = await res.json();
    const a = d.features?.[0]?.attributes;
...
async function _rdFetchHRSAHPSA(stateFips, countyFips) {
  ...
  if (_rdCacheGet(k)) return _rdCacheGet(k);
  try {
    // HRSA HPSA Primary Care - component polygons layer (11) on the public HRSA GIS portal.
    // Filter to designated (HPSA_STATUS_CD = 'D') HPSAs in the county.
    const url = `https://gisportal.hrsa.gov/server/rest/services/Shortage/HealthProfessionalShortageAreas_FS/MapServer/11/query?where=STATE_COUNTY_FIPS_CD%3D%27${fips}%27%20AND%20HPSA_STATUS_CD%3D%27D%27&outFields=HPSA_NM,HPSA_SCORE,HPSA_FTE,HPSA_DESIGNATION_POP,HPSA_ESTIMATED_UNDERSERVED_POP&returnGeometry=false&f=json&resultRecordCount=10`;
    const res = await _rdFetchWithProxy(url, { signal:_rdAbortTimeout(10000) });
    if (!res.ok) return null;
    const d = await res.json();
    const feats = d.features || [];
    ...
    const popUnderserved = designations.reduce((s, a) => s + (a.HPSA_ESTIMATED_UNDERSERVED_POP || a.HPSA_DESIGNATION_POP || 0), 0);
...
async function _rdFetchEPAAirNow(zip) {
  ...
  if (!airKey) {
    try {
      ...
      const res = await _rdFetchWithProxy(url, { signal:_rdAbortTimeout(8000) });
      if (!res.ok) return null;
