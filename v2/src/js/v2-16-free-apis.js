 ...
 */
async function v2FetchHUDRents(zip, stateAbbr) {
  // HUD FMR by state FIPS (2024 data — no API key required)
  // NOTE: zip.slice(0,2) ≠ state FIPS; must use stateAbbr→FIPS map
  const fipsMap = {
  ...
  const fips = stateAbbr ? fipsMap[stateAbbr.toUpperCase()] : null;
  if (!fips) return null;
  const cacheKey = 'hud:' + fips;
  const cached = _freeApiCache(cacheKey);
  if (cached) return cached;
  try {
