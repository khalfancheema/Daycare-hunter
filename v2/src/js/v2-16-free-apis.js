...
async function v2FetchEIAEnergy(stateAbbr) {
  ...
  try {
    ...
    const [comRes, resRes] = await Promise.all([
      fetch(`https://api.eia.gov/v2/electricity/retail-sales/data?api_key=DEMO_KEY&facets[stateid][]=${st}&facets[sectorid][]=COM&frequency=annual&data[]=price&sort[0][column]=period&sort[0][direction]=desc&length=1`, { signal: AbortSignal.timeout(6000) }),
      fetch(`https://api.eia.gov/v2/electricity/retail-sales/data?api_key=DEMO_KEY&facets[stateid][]=${st}&facets[sectorid][]=RES&frequency=annual&data[]=price&sort[0][column]=period&sort[0][direction]=desc&length=1`, { signal: AbortSignal.timeout(6000) }),
    ]);
